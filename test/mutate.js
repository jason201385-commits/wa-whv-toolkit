...
const M = [
  ...
  /* ---- /english 的結論句與顏色 ----
     這一組共同的形狀是：**面板最上面那兩個訊號（顏色、判定）跟內文各自組裝**，
     或是**結論句拿一個過期／還沒發生的日期在下定論**。
     這個站在 /english 上抓到十八次的東西全部長這樣，所以每一支都要有人釘著。 */

  /* 舊制的絕對大限：剛好相等的那一格。`capped` 碰不到（測試裡有逐日掃描證明），
     碰得到的是「你自己的 3 年剛好就是那條大限本身」——舊寫法在**比較兩個日期
     哪個早**的那一句話裡寫「比那條大限早」，而兩個日期還都印在同一行上。 */
  ["舊制大限的「剛好同一天」退回「比那條大限早」（在比日期的那句話裡說錯）",
  ...
  ["超過 60 的時候還是說「用不滿你填的那 90 天」（跟上一段自己打自己）",
    ...
    "             + (ddFloor === null\n"],
  ["夾回 60 之後不再指路，他填的那個數字就這樣消失了",
    `                ? '（你填的那 ' + ddFloor + ' 天在上面已經被壓回 ' + E.inviteDays + ' 了）'`,
    `                ? ''`],

  /* 救援句被關回 `if(saLatest)`：沒填核發日的人，面板就停在
     「往回推得到的日期已經過去了」——最後一句話是壞消息，而那句壞消息
     講的只是「排不出日期」不是「來不及」。少填一格換來更嚇人的結論。 */
  ["「排不出日期 ≠ 來不及」關回 if(saLatest)（沒填核發日就看不到）",
    "      if(saLatest || gone){", "      if(saLatest){"],
  ["沒填核發日也宣告「技術評估那一邊還開著」（替沒有資料的那一邊背書）",
    `                : '技術評估那一邊你還沒填核發日，沒有第二個日期可以跟它比——'`,
    `                : '技術評估那一邊還開著，'`],

  /* 顏色在 saGap <= 90 就轉黃，判定原本只認 saGone —— 黃燈沒有解釋。
     顏色跟判定是讀者第一眼看到的東西，它們不准指不同方向。 */
  ["判定後綴退回只認 saGone（黃燈亮著，標題還在報喜）",
    `         : saGap !== null && saGap <= 90 ? "・技術評估剩 " + saGap + " 天"\n`, ""],

  /* 考試日「還沒到」的但書掉回面板最底下：粗體結論句已經先講完，
     caveat 只是替一句已經印出去的話道歉。 */
  ["考試日的但書從最上面那一行拿掉（結論句先講完才道歉）",
    `+ esc(g.test.name) + '</b>。' + tIf + '</p>'`,
    `+ esc(g.test.name) + '</b>。' + '</p>'`],
  ["結論句不再標明它是拿一個還沒考的日期推的",
    "    const tIfShort = tFuture", "    const tIfShort = false"],

  /* `addYears` 回的是 { d, clamped }，技術評估那一邊當初只取了 d。
     英文那一邊會講「那個月沒有這一天，夾到月底」，這一邊安靜地夾——
     夾的方向往前，讀者看到的日期比他自己算的早一天而且沒有理由。 */
  ["技評的 2/29 夾日不再說明（同一條規則只有英文那邊講得出來）",
    "    const saClamp = saLatest && saLatest.clamped",
    "    const saClamp = false && saLatest.clamped"],

  /* ↓↓ 2026-08-17 第五批：**只有散文、沒有計算機**的那幾區（集簽、兩套勞資系統、
     被解僱的時效）。這一整類在今天之前零斷言——沒有函式可以餵輸入，
     golden 也只印會動的面板，於是散文自己抄的每一份數字都沒人對帳。
     flags.test.js 第 7 節是它們的第一道門，這幾條是那道門的驗收。 */

  /* 資料改了、散文沒跟上：整個站最原始的那個形狀。 */
  ["days.second 改成 90 但 howdays 照樣印 88",
    "days: { second: 88, third: 179, v:true },",
    "days: { second: 90, third: 179, v:true },"],

  /* 這一條是回歸測試，不是假想：站上原本就是這句話。
     它否認了官方兩個條件裡的第二個，照著做的人以為待滿三個月就集滿了。 */
  ["集簽標題退回「不是你上了幾天班」",
    "88 天怎麼算 —— 兩個條件都要過，日曆日只是其中一個",
    "88 天怎麼算 —— 算的是日曆日，不是你上了幾天班"],
  ["howdays 拿掉「等同全職天數」那個條件",
    "你實際上工的天數還要等同全職員工在那段期間會上的天數",
    "你要待在指定區域"],

  /* 21 個手抄的數字，抄錯一格在畫面上看不出來——20 歲那列的時薪少打一位。 */
  ["州系統年齡表 20 歲那列時薪打錯一位（$23.64→$2.64）",
    "20 歲 $898.50／$23.64／$29.55", "20 歲 $898.50／$2.64／$29.55"],
  ["成人費率在兩張卡上漂移（年齡表改了、獨資那張沒改）",
    "<b>21 歲以上 $998.30／$26.27／$32.84</b>",
    "<b>21 歲以上 $997.30／$26.24／$32.80</b>"],

  /* 「打去問就知道」對非營利那一格不成立——官方明講它自己也判不了。
     少了這個例外，這個面板會叫人去打一支不會給答案的電話。 */
  ["「怎麼判斷」把 Wageline 講成萬用解（拿掉非營利那個例外）",
    "<b>唯一的例外就是上面那一格</b>：非營利／立案協會屬於哪一套，Wageline 明言不提供意見。",
    ""],

  /* 時效是這一區唯一「過了就沒了」的東西。只給一個數字＝叫人押錯機關。 */
  ["時效那張卡只留全國那一套（州系統的 28 天不見）",
    "期限<b>28 天</b>", "期限<b>不限</b>"],
  ["檢舉區結語退回「先問問看」開頭（被解僱的人第一句話讀到慢慢來）",
    "<b>已經被解僱的人先看上面那條「時鐘」——21 天／28 天是硬的，先把件送出去，再慢慢弄清楚細節。</b>其他情況不必緊張：<b>可以匿名檢舉</b>",
    "<b>可以匿名檢舉</b>"],
];
