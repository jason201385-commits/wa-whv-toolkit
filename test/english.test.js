...
/* capped 碰不到，但**剛好相等**碰得到，而那正是原本說錯話的那一格：
   舊制最後一天（E.t2.until）考的成績，自己的 3 年就是那條大限本身，
   原文卻寫「比那條大限早」。錯在「在講兩個日期哪個早」的那一句裡，
   讀者手上沒有別的線索可以校正——兩個日期還都印在同一行上。 */
{
  const edge = run({ date: E.t2.until, test: "pte", sc: [65, 65, 65, 65] });
  ok("舊制最後一天考的：兩條線同一天到期，不准說「比那條大限早」",
    edge.html.includes("同一天到期") && !edge.html.includes("比那條大限早"),
    (edge.html.match(/你自己的 3 年[^<]*/) || [""])[0]);
  ok("而且那一行要同時印出自己的到期日與那條大限（讀者才對得起來）",
    edge.html.includes("你自己的 3 年也剛好在 " + E.oldExpiry));
}
...
/* 2/29 核發的評估，加 3 年那個月沒有這一天。英文那一邊講得出來，
   技術評估那一邊當初把 addYears 回傳的 clamped 丟掉了——同一條規則只有一邊會講。 */
{
  const leap = run({ date: "2026-01-01", test: "ieltsA", sc: [7, 7, 7, 7], sa: "2024-02-29" });
  ok("2/29 核發的評估：夾到 2/28，而且要講出來為什麼",
    leap.text.includes("2027-02-28") && leap.text.includes("那個月沒有這一天"),
    (leap.text.match(/技術評估這一邊[^\n]*/) || [""])[0].slice(0, 220));
  const plain29 = run({ date: "2026-01-01", test: "ieltsA", sc: [7, 7, 7, 7], sa: "2024-03-01" });
  ok("沒被夾到的核發日不准冒出這段解釋",
    !plain29.text.includes("那個月沒有這一天"));
}

/* 舊制、成績還有 15 天、遞件天數填 90（會被夾到 60）、**沒填核發日**。
   一格情境同時踩四個修法裡的兩個：複述輸入用的是夾完的值(#10)、
   救援句被關在 `if(saLatest)` 裡面(#8)。 */
{
  const g90 = run({ date: "2023-09-01", test: "pte", sc: [65, 65, 65, 65], delay: 90 });
  ok("複述使用者輸入用 ddFloor：填 90 就寫 90，不准寫夾完的 60",
    g90.text.includes("你填的那 90 天") && !g90.text.includes("你填的那 60 天"),
    (g90.text.match(/用不滿[^\n]*/) || [""])[0]);
  /* 但也不准反過來說「用不滿你填的那 90 天」——往回推扣掉的是 60，
     而正上方那一段才剛說「填再大的數字都沒有意義」。 */
  ok("超過 60 的時候，被「用不滿」的那個數字要是 60，不是他填的 90",
    g90.text.includes("用不滿 60 天的邀請期") && !g90.text.includes("用不滿你填的那 90 天"),
    (g90.text.match(/用不滿[^\n]*/) || [""])[0]);
  ok("而他填的 90 要以指路的形式留著，不然他會不知道自己那格去哪了",
    g90.text.includes("你填的那 90 天在上面已經被壓回 60 了"));
  /* 沒被夾到的那一格才是「你填的那 N 天」原本要服務的情況。 */
  const g30 = run({ date: "2023-09-01", test: "pte", sc: [65, 65, 65, 65], delay: 30 });
  ok("沒超過 60 就照他填的講，而且不准冒出那句指路",
    g30.text.includes("用不滿你填的那 30 天") && !g30.text.includes("已經被壓回"),
    (g30.text.match(/用不滿[^\n]*/) || [""])[0]);
  ok("沒填核發日也要印「排不出日期 ≠ 來不及」——少填一格不該換來更嚇人的結論",
    g90.text.includes("但這不等於來不及"), g90.text.split("\n").slice(-2).join(" ⏎ "));
  ok("而且不准替沒填的那一邊宣告「技術評估那一邊還開著」",
    !g90.text.includes("技術評估那一邊還開著")
      && g90.text.includes("技術評估那一邊你還沒填核發日"));
  /* 正上方那一行（saLine）已經說過「兩個都填進來才看得出哪一扇門先關」。
     結論句再講一次就是把同一句話貼兩遍，這個站的註解在四個地方罵過這件事。 */
  ok("那句「才看得出哪一扇門先關」在整個面板裡只准出現一次",
    (g90.text.match(/才看得出哪一扇門先關/g) || []).length === 1,
    "出現 " + (g90.text.match(/才看得出哪一扇門先關/g) || []).length + " 次");
}
{
  /* 對照組：同樣的情境填了還有效的核發日，那一句才可以出現。 */
  const g90sa = run({ date: "2023-09-01", test: "pte", sc: [65, 65, 65, 65], delay: 90, sa: "2025-01-01" });
  ok("填了核發日的同一格，才輪到「技術評估那一邊還開著」",
    g90sa.text.includes("技術評估那一邊還開著")
      && g90sa.text.includes("你填的那 90 天"));
}

/* 顏色在 saGap <= 90 就轉黃，標題原本只認 saGone——黃燈沒有解釋。 */
{
  const near30 = run({ date: "2026-01-01", test: "ieltsA", sc: [7, 7, 7, 7], sa: "2023-09-16" });
  ok("技術評估剩 30 天：顏色是黃的", near30.tone === "warn", "tone=" + near30.tone);
  ok("而且標題要指名是誰造成的黃燈，不能只印「Proficient　10 分」",
    /技術評估剩 \d+ 天/.test(near30.verdict), "verdict=" + near30.verdict);
  const far = run({ date: "2026-01-01", test: "ieltsA", sc: [7, 7, 7, 7], sa: "2026-01-01" });
  ok("評估還很久就不要加這個後綴（加了會變成沒事也在示警）",
    far.tone === "ok" && !/技術評估剩/.test(far.verdict), "verdict=" + far.verdict);
}

/* 考試日還沒到的但書要跟著數字走，不是排在粗體結論句後面替它道歉。 */
{
  const fut = run({ date: "2026-12-01", test: "ieltsA", sc: [7, 7, 7, 7], sa: "2025-01-01" });
  ok("「考試日期還沒到」要黏在最上面那個日期後面，不是掉在面板最底下",
    fut.html.includes("</b>。（提醒：這個考試日期還沒到"),
    (fut.text.split("\n")[0] || "").slice(0, 160));
  const iCav = fut.text.indexOf("這個考試日期還沒到");
  const iEnd = fut.text.indexOf("先關的是");
  ok("而且要排在結論句**之前**（讀者讀到粗體結論就停了）",
    iCav >= 0 && iEnd >= 0 && iCav < iEnd, "caveat@" + iCav + " 結論@" + iEnd);
  ok("結論句自己也要留一句指路，不然它是拿一個還沒發生的日期下的定論",
    fut.text.includes("全文見最上面那一行"), fut.text.split("\n").slice(-1)[0]);
  /* 舊寫法的痕跡：那一段當初寫「上面照算」，因為它真的在最下面。 */
  ok("舊的面板底部寫法要整個消失", !fut.text.includes("上面照算"));
  ok("考試日期沒問題的面板不准冒出這個但書",
    !prof.text.includes("這個考試日期還沒到") && !prof.text.includes("全文見最上面那一行"));
}
