...
(function () {
  ...
  /* ⚠️ 這兩格補的是**閱讀順序**，而順序正是這一層在守的東西——
     斷言問「這句話在不在」，只有逐字快照答得出「它排在第幾行」。
     上面那 27 格裡沒有一格的考試日在未來，也沒有一格同時 `gone` 又填了 >60 天，
     所以那兩個 bug 在這一層完全隱形。 */
  scene("考試日還沒到（但書要黏在最上面那個日期後面，不是掉在最底下）",
    { date: "2026-12-01", test: "ieltsA", sc: [7, 7, 7, 7], sa: "2025-01-01" });
  scene("排不出邀請日＋遞件天數填 90（複述輸入不准用夾完的 60）",
    { date: "2023-09-01", test: "pte", sc: [65, 65, 65, 65], delay: 90 });
  /* 2/29 那一格上面已經有一個（考試日），但技術評估那一邊當初把 clamped 丟掉了，
     所以要有一格是「考試日正常、核發日才是 2/29」——不然兩邊的漏洞會互相遮住。 */
  scene("技評核發日是 2/29（夾到 2/28，而且要說得出為什麼）",
    { date: "2026-01-01", test: "ieltsA", sc: [7, 7, 7, 7], sa: "2024-02-29" });
})();
