...
零相依套件，合計 755 個案例（時薪 28、生活成本 388、點數 71、定居 106、旗標對帳 15、英文 147）。六支都**直接從 `index.html` 挖出 `DATA` 與要測的函式用 `vm` 跑**，所以測到的一定是正式版程式碼，不是抄一份出來的副本（抄出來的副本會在你改了 `index.html` 之後繼續騙你說全過）。

另外四支不算進上面的數字，因為它們驗的不是網站，是**上面那六支到底有沒有在測東西**：

```bash
node test/golden.test.js   # 快照：66 個場景的答案面板逐字比對（含剪貼簿那一份）
