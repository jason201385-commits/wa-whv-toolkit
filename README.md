...
零相依套件，合計 591 個案例（時薪 28、生活成本 383、點數 71、定居 94、旗標對帳 15）。五支都**直接從 `index.html` 挖出 `DATA` 與要測的函式用 `vm` 跑**，所以測到的一定是正式版程式碼，不是抄一份出來的副本（抄出來的副本會在你改了 `index.html` 之後繼續騙你說全過）。

另外四支不算進上面的數字，因為它們驗的不是網站，是**上面那五支到底有沒有在測東西**：

```bash
node test/golden.test.js   # 快照：36 個場景的答案面板逐字比對（含剪貼簿那一份）
node test/mutate.js        # 突變測試：改壞一個字，看測試會不會紅（目前 50 條全被抓到）
