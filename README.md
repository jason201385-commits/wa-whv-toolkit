...
零相依套件，合計 340 個案例（時薪 28、生活成本 132、點數 71、定居 94、旗標對帳 15）。五支都**直接從 `index.html` 挖出 `DATA` 與要測的函式用 `vm` 跑**，所以測到的一定是正式版程式碼，不是抄一份出來的副本（抄出來的副本會在你改了 `index.html` 之後繼續騙你說全過）。

另外兩支不算進上面的數字，因為它們驗的不是網站，是**上面那五支到底有沒有在測東西**：

```bash
node test/mutate.js     # 突變測試：改壞一個字，看測試會不會紅（目前 16 條全被抓到）
...
同一天在 340 條全綠的狀態下，讀渲染輸出抓到兩個缺陷——`/settle` 有一句話指著一個
