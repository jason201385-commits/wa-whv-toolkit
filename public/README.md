...
test/
  ...
  cost.test.js        生活成本試算的回歸測試（97 個案例）。`node test/cost.test.js`
...
```bash
grep -o 'v:false[},]' public/index.html | wc -l    # 21
...
**x，剩下的 3 筆不住在 `.src` 裡**——它們是沒有網址的內容卡片，由別的 renderer 印出「待核」：`DATA.wage.report.calls[1]`（Fair Work 口譯服務）、`DATA.rent.exit[2]`（退租還要先給通知期）、`DATA.rent.exit[3]`（所以離開西澳前 6–8 週就要開始跑）。
...
```js
document.querySelectorAll('.pending').length   // 21 + 1 = 22
```
