...
| `/settle` | 定居的兩個時鐘 | **兩個時鐘的起點不一樣，混在一起會錯得很貴**：戶籍與健保那條從**離台日**起算（出境滿 2 年應為戶籍遷出登記 → 退保），駕照那條從 **PR 核准日**起算（3 個月內換照，不是落地起算）。三件在中文網路上普遍還是舊版的事實：**台灣自 2025-11-01 起不在西澳的經驗駕駛承認名單上**（換照要考筆試＋路考，實付從 A$63.50 變 A$173.20）、**台灣健保的停保制度自 2024-12-23 起廢止**（憲法法庭 111 年憲判字第 19 號）、**台灣不在 Medicare 互惠協定的 11 國名單裡**。另外標出整條路上唯一不可逆的一格：**PR 核准當天，超級年金的離境提領（DASP）資格永久消失**。 |
...
test/
  ...
  cost.test.js        生活成本試算的回歸測試（100 個案例）。`node test/cost.test.js`
  pr.test.js          年齡時鐘與點數試算的回歸測試（71 個案例）。`node test/pr.test.js`
  settle.test.js      定居兩個時鐘與換照費用的回歸測試（64 個案例）。`node test/settle.test.js`
  flags.test.js       旗標對帳（15 個案例）。`node test/flags.test.js`
                      五支都零相依套件，都直接從 index.html 挖出 DATA 與要測的函式來跑，
                      所以測到的一定是正式版程式碼，不是抄一份出來的副本。合計 278 個案例。
...
| **2027-01-05** | Jason | ⭐ **台灣是否被放回西澳的經驗駕駛承認名單**（`DATA.settle.drive`）——這是 2025-11-01 才生效的移出，站上整段的前提就是它 | `transport.wa.gov.au` 的 recognised countries 頁。⚠️ **官方那個國家查詢器對台灣仍顯示舊文案**，要看的是它回傳的分類，不是那句話；站上已就這個矛盾做了揭露，**名單真的改回來時要連那段揭露一起刪** |
...
| **每次動 `/settle` 的日期或文案時** | 改的人 | `node test/settle.test.js` 有沒有全過 | 同上。它連**文案**都鎖（2025-11-01、2024-12-23、11 國、DASP），所以「順手改順一點」也會紅——**那是刻意的**，這幾句話是這一區存在的理由 |
