...
test/
  ...
  cost.test.js        生活成本試算的回歸測試（132 個案例）。`node test/cost.test.js`
  ...
  flags.test.js       旗標對帳（15 個案例）。`node test/flags.test.js`
                      ...
                      所以測到的一定是正式版程式碼，不是抄一份出來的副本。合計 340 個案例。
  ...
  mutate.js           **驗收測試本身的工具**（不算進上面的 340）。`node test/mutate.js`
                      它故意改壞 index.html 的一個字，看五支測試會不會紅——全綠代表
                      那條斷言是裝飾品。目前 16 條突變全部被抓到（0 漏網）。
  ...
  render.js           **把答案面板印成純文字給人讀**（唯讀）。`node test/render.js`
                      ...
                      斷言證明「該出現的字串有出現」，證明不了整段話讀起來對不對——
                      2026-08-17 抓到的兩個缺陷都是這樣看出來的（見下方複核日曆）。
