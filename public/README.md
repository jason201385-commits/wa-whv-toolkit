...
test/
  ...
  pr.test.js          年齡時鐘與點數試算的回歸測試（51 個案例）。`node test/pr.test.js`
  flags.test.js       旗標對帳（15 個案例）。`node test/flags.test.js`
                      四支都零相依套件，都直接從 index.html 挖出 DATA 與要測的函式來跑，
                      所以測到的一定是正式版程式碼，不是抄一份出來的副本。
                      前三支對應的是**會主動說出改變使用者行為的話**的路徑
...
**剩下的 3 筆不住在 `.src` 裡**——它們是沒有網址的內容卡片，由別的 renderer 印出「待核」：`DATA.wage.report.calls[1]`（Fair Work 口譯服務）、`DATA.rent.exit[2]`（退租還要先給通知期）、`DATA.rent.exit[3]`（所以離開西澳前 6–8 週就要開始跑）。
